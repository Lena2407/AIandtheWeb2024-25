...
quantity = pd.DataFrame(scores, index=["quantity"])
quantity = quantity.sort_values(["quantity"], axis = 1)
with col1:
    st.subheader("Scoreboard by quantity of guesses")
    st.dataframe(quantity.T)


quality =  scores.iloc[1]
avg_qual = {}
